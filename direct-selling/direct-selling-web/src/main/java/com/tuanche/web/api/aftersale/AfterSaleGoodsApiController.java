...
@RestController
@RequestMapping("/api/after/sale/goods")
public class AfterSaleGoodsApiController {
    ...
    @Reference
    private AfterSaleActivityPackageLabelService afterSaleActivityPackageLabelService;
...
//            }
            ...
            queryRequestDto.setPageSize(500);
            ...
            Map<Integer,List<AfterSaleActivityPackageLabelDto>> labelMap = afterSaleActivityPackageLabelService.getPackageLabelNamesByPackageIds(packageIds);
...
//                });
                ...
                activityPackageList.forEach(pd->{
                    Integer goodsId= pd.getGoodsId();
                    CommodityResponseDto v = goodsMap.get(goodsId);
                    if(v != null){
                        AfterSaleActivityPackageDto packageDto = new AfterSaleActivityPackageDto();
                        packageDto.setGoodsId(v.getCommodityId());
                        ...
                        if(labelMap!=null){
                            packageDto.setLabels(labelMap.get(pd.getId()));
                        }
                        packageDtos.add(packageDto);
                    }
                });
