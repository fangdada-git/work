...
import com.tuanche.web.vo.AfterSaleAgentOrderVo;
import com.tuanche.web.vo.AfterSaleAgentStatDetailVo;
import com.tuanche.web.vo.AfterSaleAgentStatSearchVo;
import com.tuanche.web.vo.AfterSaleAgentStatVo;
import com.tuanche.web.vo.AfterSaleWriteOffStatisticsSearchVo;
import com.tuanche.web.vo.AfterSaleWriteOffStatisticsVo;
...
@RestController
public class AfterSaleStatController extends BaseController {
...
//        }
     ...
     * @return
     */
    @PostMapping("/share/stat/detail")
    public TcResponse shareStatDetail(@RequestBody AfterSaleAgentStatSearchVo statSearchVo) throws ParseException {
        long st = System.currentTimeMillis();
        if (statSearchVo.getActivityId() == null) {
            return paramInvalid(st);
        }
...
//        }
        ...
        List<AfterSaleAgentStatDetailDto> afterSaleAgentStatDetailDtos = afterSaleStatService.selectShareStatDetail(statSearchVo.getActivityId(), sdf.parse(statSearchVo.getStartDateTime()), sdf.parse(statSearchVo.getEndDateTime()));
        List<AfterSaleAgentStatDetailVo> list = new ArrayList<>(afterSaleAgentStatDetailDtos.size());
        AfterSaleAgentStatDetailVo statDetailVo = null;
        for (AfterSaleAgentStatDetailDto statDetailDto : afterSaleAgentStatDetailDtos) {
            statDetailVo = new AfterSaleAgentStatDetailVo();
            statDetailVo.setDateTime(statDetailDto.getDateTime());
            statDetailVo.setShareCount(statDetailDto.getShareCount());
            statDetailVo.setPackageCardCount(statDetailDto.getPackageCardCount());
            statDetailVo.setUniqueVisitorCount(statDetailDto.getUniqueVisitorCount());
            statDetailVo.setConversionRate(statDetailDto.getConversionRate());
            list.add(statDetailVo);
        }
        return success(list, st);
    }
